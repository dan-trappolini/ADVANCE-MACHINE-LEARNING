...
# The parameter space we are going to explore
param_grid = {
    "hidden_size" : np.arange(100, 400, 100), 
    "niter" : np.arange(3000, 5000, 1000),
    "batch":[128, 256, 512] , 
    "lerate": [1e-2, 1e-3, 1e-4], 
    "reg" : [1e-2, 1e-3, 1e-4]}
...
for comb in combinations:
    ...
    results_validation.append(val_acc)
    results_train.append(train_acc)
    
    
    # create a dictionary with all results for all combinations
    all_combo[comb] = stats


# Save the dictionary with all the results as a txt
with open(r'test.txt','w+') as f:
    f.write(str(all_combo))
...
# Best combination
best_comb = combinations[results_validation.index([max(results_validation)])]
final_results = list(zip(combinations,results_train, results_validation))

# Save the results in a csv
pd.DataFrame(final_results).rename(columns = {0:"combinations",1:"train accuracy",2:"val accuracy"}).to_csv("results.csv")

df_results = pd.DataFrame(final_results).rename(columns = {0:"combinations",1:"train accuracy",2:"val accuracy"})

# Order by train accuracy
...
# Bottom 10 results
df_results_bot10 = df_results.tail(10)


# Save the results to latex
with open('top10.tex', 'w') as tf:
     tf.write(df_results_top10.to_latex())
     

# Save the results to latex
with open('bot10.tex', 'w') as tf:
     tf.write(df_results_bot10.to_latex())

## -------------Best Combination overall--------------
import time
start = time.time()

combo = list(best_comb)
np.random.seed(123)
best_net = TwoLayerNet(input_size, combo[0], num_classes)
stats = best_net.train(X_train, y_train, X_val, y_val,
            num_iters=combo[1], batch_size=combo[2],
            learning_rate=combo[3], learning_rate_decay=0.95,
            reg=combo[4], verbose=True)
print("code execution time: ",time.time() - start)

# Predict on the validation set
print('-----------')
print('hidden size: ',combo[0], ', num_iters: ',combo[1], ', batch size: ',combo[2], ', learning_rate: ', combo[3], ', regula: ',combo[4] )
